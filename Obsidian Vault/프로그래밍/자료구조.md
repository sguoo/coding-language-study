...
# 정렬
## 퀵정렬
1. 양 끝부터 시작해, 왼쪽과 오른쪽을 비교한다. 만약 오른쪽이 왼쪽과 같다면, 위치를 바꾼다.
2. 만약 위치가 바뀌었다면, 원래 나와있던게 계속 나와있고, 2번째 왼쪽과 비교한다
3. 만약 위치가 바뀌지 않았다면, 방향은 그대로, 2번째와 비교한다.
4. 만약 위치가 정렬이 되었다면, 그 숫자를 기준으로 하여 나눈다.
5. 왼쪽부터 1번을 다시 시작한다.
6. 왼쪽이 모두 정렬이 되었다면, 왼쪽에서 2번째에서 1번을 다시 시작한다