...
- 메서드를 통해 객체의 행동을 정의하고, 인스턴스를 사용해 상태를 표현할 수 있다.
>[!warning] 클래스 내부에 선언된 함수를 메서드라 부른다. <br>보통 함수가 메서드보다 포괄적인 의미를 가지기도 하니 헷갈림을 주의해야한다.
- 메서드는 일반 함수와 비슷하게 def키워드를 사용해 정의한다
- 인자를 사용하는 방식도 비슷하다
- 단, 첫 번째 인자는 반드시 self로 써야함
```python
class 클래스명:
	def 메서드명(self, 인자):
		코드
...
```
1. Calc 클래스의 생성자를 정의한다. 생성자도 메서드 이므로 일반 함수와 같은 특성을 갖는다. self 키워드를 첫번째 인자로 사용하지 않으면 클래스는 내부에 있는 메서드로 인식하지 못한다.
2. 생성자 함수 내부, 생성자에서 받은 인자로 인스턴스 변수를 초기화한다. 인스턴스 변수로 사용하려면 self 키워드를 사용한다
3. 덧셈 메서드 정의
4. self.value에 인자로 받은 n의 값을 더해서 반환한다. self.value는 인스턴스 변수이기 때문에 덧셈 메서드에서 정의하지 않아도 사용 가능
5. Calc 클래스의 인스턴스를 생성하면서 초깃값으로 100을 입력했다. 입력받은 초깃값은 생성의 인자, init_value에 저장된다. 첫번째 인자인 self에 저장되는 것이 아니다. self는 인스턴스 내부에서 자신을 참조하기 위해서 사용된다
6. calc 인스턴스 변수 value의 값을 format( ) 함수를 이용해 출력한다
▶ 결과
```python
value = 100
